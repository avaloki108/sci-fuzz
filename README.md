...
- **Exploit-grade economic oracles** ([`economic.rs`](src/economic.rs)), all on execution evidence (logs + per-tx `state_diff`, plus **sequence-cumulative logs** where noted):
  ...
  - ERC-20 balance mapping writes with no `Transfer` in the same tx (callback / direct-write signal).
  - ERC-4626 exchange-rate **jumps** between consecutive **`Deposit`** events and consecutive **`Withdraw`** events (cumulative log stream; default 5× threshold).
  - ERC-4626 **same-transaction multi-`Deposit` rate spread** — two or more `Deposit` events from one vault in a **single** transaction with inconsistent implied assets/share rates (read-only reentrancy / stale pricing / broken mint path signal).
  ...
  - **Reserve / conservation:** lightweight **supply-side** checks only (mint/burn vs `totalSupply` write); there is **no** full pool reserve conservation or AMM invariant engine yet.
  - Helps ERC-4626 vaults, ERC-20 accounting, and lending-style layouts only where these patterns appear in logs/storage — **not** deep protocol semantics or fork realism.
