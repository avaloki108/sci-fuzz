...
impl Invariant for Erc20BurnWithoutSupplyWriteOracle {
    ...
    fn check(
        ...
        for log in &result.logs {
            ...
            return Some(Finding {
                severity: Severity::Critical,
                title: format!("Economic: ERC-20 large burn without totalSupply storage update ({token})"),
                description: format!(
                    "Token {token} logged a large burn (to zero) of {value} units but slot {} (_totalSupply heuristic) was not written this transaction — supply accounting may be inconsistent with balances (value destruction / ledger break).",
                    self.total_supply_slot
                ),
                contract: token,
                reproducer: sequence.to_vec(),
